...
import JobListSidebar from "../job-list/[place]/JobListSidebar";
...
const JobCategoryListData = ({filteredJob}) => {
...
const records=filteredJob.slice(firstIndex,lastIndex)
let number=[];
 if(filteredJob.length>0){
   number=[...Array(Math.ceil(filteredJob?.length/itemPerPage)).keys()].slice(1)
}
...
useEffect(()=>{
  ...
  return (
    <div className='w-full h-full flex flex-col items-center space-y-9 justify-center'>
      ...
      <div className='w-full flex flex-col space-y-3 items-start'>
          ...
          <div className='flex flex-col  space-y-2  md:items-center md:space-y-0 md:flex-row md:justify-between w-full '>
            <div style={{fontWeight:100}} className={`${jost.className}`}>Showing {firstIndex} - {lastIndex<filteredJob.length? lastIndex :filteredJob.length} of {filteredJob.length} result</div>     
            <div className='flex flex-col md:flex-row  space-y-2 md:space-y-0 md:space-x-3  items-start justify-start '>
            </div>
          </div>
           </div>
    ...
    <div className='w-full grid grid-cols-1  space-y-5'>
       ...
       <Link href={`/job/${data.url}`} key={index}  className='w-full flex items-start justify-between  border border-slate-200 rounded-md   group '>
        <div className=' flex items-center -center space-x-4 p-5 h-full  '>
         {/* //!  Image */}
          <div className=' h-full'><Image src={`/${data.icon}.png`}  height={50} width={50} alt='Image/Logo' /></div>
         {/* //! Data */}
    ...
    <div className='min-w-full  flex items-center justify-center'>
      ...
      {filteredJob.length>0 && number.map((data,index)=>
...
export default JobCategoryListData